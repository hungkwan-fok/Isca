...
# start parameter sweep (rotation)
earth_omega = 7.292e-5
scales = [1/2, 5/8, 3/4, 7/8, 1, 5/4, 3/2, 7/4, 2]  

eps_list = [0, 5, 10, 15, 20, 25, 30]
...
if __name__ == "__main__":
    ...
    for s in scales:
        for eps in eps_list:
            exp_name = f'hs_eps_{eps}_omega_{s}'
            omega = earth_omega * s

            exp = Experiment(exp_name, codebase=cb)
            exp.set_resolution(*RESOLUTION)

            nl = base_namelist.copy()
            exp.namelist = nl
            exp.diag_table = diag
            exp.update_namelist({"constants_nml": {"omega": omega}})
            exp.update_namelist({"hs_forcing_nml": {"eps": eps}})
            ...
            try:
                ...
                print(f"[WARN] Run failed for scale={s}; continuing to next value.")
                continue
